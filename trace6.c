...
void check_packets(u_char *foo, const struct pcap_pkthdr *header, const unsigned char *data) {
  ...
  if (tunnel && ptr[6] == NXT_ICMP6 && ptr[40] == ICMP6_TOOBIG && len >= 100) {
    ...
    if (new_mtu < mtu) {
      if (type != 2) {
        pos = ptr[94 + 8*(do_alert + do_frag + do_dst)];
        pos2 = ptr[93 + 8*(do_alert + do_frag + do_dst)];
      } else {
        pos = ptr[101 + 0x30 + 8*(do_alert + do_frag + do_dst + do_toobig - 1)];
...
//printf(" POS1 is : 94 + 8*(do_alert + do_frag + do_dst) = [%d] == %d <= %d ?\n", pos, up_to);
//printf(" POS2 is : 93 + 8*(do_alert + do_frag + do_dst) = [%d] == %d <= %d ?\n", pos2, up_to);
      } else {
        pos = ptr[101 + 0x30 + 8*(do_alert + do_frag + do_dst + do_toobig - 1)];
        pos2 = ptr[100 + 0x30 + 8*(do_alert + do_frag + do_dst + do_toobig - 1)];
      }
...
//        if (position[pos + 1][0] == '?')
    ...
    if (ptr[6] == NXT_ICMP6 && ptr[40] == ICMP6_TTLEXEED && ptr[41] == 0 && len >= 100) {
      si = (unsigned short int *) &ptr[88 + 8*(do_alert + do_frag + do_dst)];
      ui = (unsigned int *) &ptr[92 + 8*(do_alert + do_frag + do_dst)];
      pos = htons((*si % 65536)) - baseport;
      if (pid != htonl(*ui))
        return;
      if (pos <= up_to)
        ...
        if (position[i] != NULL && position[i][0] == '?') {
          ...
          if (do_alert)
            if (thc_add_hdr_hopbyhop(pkt, &pkt_len, buf3, 6) < 0)
              return -1;
          if (type != 2) {
            if (do_frag) for (m = 0; m < do_frag; m++)
          ...
          if (type == 0) {
            ...
            if (type == 2) {
              ...
              if (foo6[8])
                ...
                foo6[8] = 1;
              if ((pkt2 = thc_create_ipv6(interface, 0, &pkt2_len, dst6, foo6, i, 0, 0, 0, 0)) == NULL)
                return -1;
